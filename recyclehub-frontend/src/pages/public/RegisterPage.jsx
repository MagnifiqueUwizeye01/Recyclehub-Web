...
const BUYER_STEPS = ['Choose Role', 'Personal Info'];
const SELLER_STEPS = ['Choose Role', 'Personal Info', 'Company Info'];
...
export default function RegisterPage() {
  ...
  const steps = useMemo(
    () => (role === 'Seller' ? SELLER_STEPS : BUYER_STEPS),
    [role]
  );
  ...
  const completeRegistration = async (data, companyFields = {}) => {
    if (!data?.username || !data?.password || data?.confirmPassword === undefined) {
    ...
    try {
      ...
      const file = companyFields.licenseFile;
      if (isSeller && file instanceof File && file.size > 0) {
        licenseDocument = await uploadSellerLicense(file);
      }

      const websiteRaw = (companyFields.websiteUrl || '').toString().trim();
      const payload = {
  ...
  const onSellerCompanySubmit = async (e) => {
    e.preventDefault();
    const fd = new FormData(e.target);
    await completeRegistration(personalData, {
      companyName: (fd.get('companyName') || '').toString().trim(),
      city: (fd.get('city') || '').toString().trim(),
      address: (fd.get('address') || '').toString().trim(),
      websiteUrl: fd.get('websiteUrl'),
      licenseFile: fd.get('licenseDocument'),
    });
  };
    ...
    <div className="min-h-screen bg-hub-bg flex">
      <div className="hidden lg:flex w-2/5 bg-hub-surface border-r border-hub-border flex-col justify-center px-12 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-hub-muted font-body mb-8">
            Connect with verified buyers and sellers of industrial recyclable materials across Africa.
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-md">
              ...
              <div key={s} className="flex items-center gap-2 flex-1">
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-mono font-bold transition-all ${
                    i <= step ? 'bg-hub-accent text-white' : 'bg-hub-surface2 text-hub-muted border border-hub-border'
                  }`}
                >
                ...
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 ${i < step ? 'bg-hub-accent' : 'bg-hub-border'}`} />
                )}
              </div>
            ))}
          </div>
                ...
                {[
                  {
                    ...
                    title: "I'm a Buyer",
                    desc: 'Browse and purchase verified recyclable materials. No company details needed to start.',
                  },
                  {
                    r: 'Seller',
                    icon: <Building2 size={28} />,
                    title: "I'm a Seller",
                    desc: 'List materials for sale. You will provide company and license details next.',
                  },
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-hub-accent/10 flex items-center justify-center text-hub-accent group-hover:scale-110 transition-transform">
                        {icon}
                      </div>
                      <div>
            ...
            <form onSubmit={handleSubmit(onPersonalSubmit)} className="space-y-4">
              {isSeller && (
                <div className="rounded-xl bg-gray-50 border border-gray-200 px-4 py-3 text-sm text-gray-700">
                  Next step: company and license details for seller verification.
                </div>
              )}
              {registrationError && (
              ...
              <div>
                <Input
                  ...
                  rightElement={
                    <button type="button" onClick={() => setShowPwd((p) => !p)} className="text-hub-muted hover:text-hub-text">
                ...
                {password && (
                  <div className="flex gap-1 mt-1">
                    {[0, 1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className={`h-1 flex-1 rounded-full ${i < strength ? strengthColors[strength - 1] : 'bg-hub-border'}`}
                      />
                    ))}
                  </div>
                )}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="submit" loading={loading && !isSeller} className="flex-1">
                  {isSeller ? 'Continue' : 'Create account'}
                </Button>
              </div>
            </form>
          )}
          ...
          {step === 2 && isSeller && (
            <form onSubmit={onSellerCompanySubmit} className="space-y-4">
              {registrationError && (
                <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 flex items-start gap-2">
                  <AlertCircle size={16} className="text-red-500 shrink-0 mt-0.5" />
                  <p className="text-sm text-red-700">{registrationError}</p>
              ...
              <h1 className="text-2xl font-display font-bold text-hub-text mb-1">Company Information</h1>
              <p className="text-sm text-hub-muted mb-4">
                Required for seller verification before your listings go live.
              </p>
              ...
              />
              <div>
                <label className="text-sm font-medium text-hub-textMuted font-body block mb-1.5">
                  License Document
                </label>
                <input
                  type="file"
                ...
                />
                <p className="mt-1 text-xs text-hub-muted">Upload your business license for admin review.</p>
              </div>
              <div className="flex gap-3 pt-2">
          ...
          <p className="text-center text-sm text-hub-muted mt-6 font-body">
            ...
            <Link to="/login" className="text-hub-accent hover:underline">
              Sign in
            </Link>
          </p>
        </div>
